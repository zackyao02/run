...
export function ProgramRunner({ run, source, sessionEndpoint, usageEndpoint, prototype = false }: Props) {
  ...
  const minutes = String(Math.floor(timer / 60)).padStart(2, "0"); const seconds = String(timer % 60).padStart(2, "0");
    ...
    return <><>{showCompileIntro && <section className="compile-intro" aria-live="polite"><div className="compile-intro-card"><span className="compile-intro-kicker">已编译知识程序 · 正在装载</span><h2>正在准备这次运行</h2><p>正在校验来源证据，并装载发布前确认的规则路径。</p><CompileLoadingVisual step={compileStep} /><div className="compile-flow">{[["校验知识来源", "确认文章段落与证据"], ["装载规则路径", "恢复已确认的状态与分支"], ["启动知识程序", "准备本次输入与结果"]].map(([title, note], index) => <div className={`compile-flow-step compile-flow-step-${index + 1} ${index <= compileStep ? "active" : ""}`} key={title}><i>{index < compileStep ? "✓" : index + 1}</i><div><strong>{title}</strong><small>{note}</small></div></div>)}</div><div className="compile-progress"><i style={{ width: `${((compileStep + 1) / 3) * 100}%` }} /></div><small className="compile-note">知识程序已在发布前确认；本次只装载并运行已确认的原文规则。</small></div></section>}</><main className="shell demo-run-shell program-shell"><header className="topbar demo-topbar"><Link href="/" className="brand">用<span>一下</span></Link><div className="demo-top-label">{prototype ? "产品体验样例" : "知乎知识"}</div><div className="demo-top-actions">{!prototype && <ZhihuNavStatus />}{prototype ? <div className="source-nav-link">知识正在运行</div> : <Link className="source-nav-link" href={`/knowledge/${run.sourceRef.sourceId}`}>阅读原文 ↗</Link>}</div></header><div className="container demo-run-container">
