...
function modelName(model?: ExecutableRun["program"] extends infer _T ? string : never) {
...
export function ProgramRunner({ run, source, sessionEndpoint, usageEndpoint, prototype = false }: Props) {
  ...
  useEffect(() => {
    ...
    const done = window.setTimeout(() => setShowCompileIntro(false), 1050);
  ...
  const minutes = String(Math.floor(timer / 60)).padStart(2, "0"); const seconds = String(timer % 60).padStart(2, "0");
    ...
    return <><>{showCompileIntro && <section className="compile-intro" aria-live="polite"><div className="compile-intro-card"><span className="compile-intro-kicker">整篇知识正在变成可操作程序</span><h2>正在准备这次运行</h2><p>文章已经预编译完成；现在装载这篇文章专属的规则、分支和结果物。</p><div className="compile-flow">{[["读取整篇文章", "已载入完整原文段落"], ["定位条件与动作", "可运行规则已经绑定原文依据"], ["装载定制程序", "本次填写将改变状态、路径和结果"]].map(([title, note], index) => <div className={`compile-flow-step ${index <= compileStep ? "active" : ""}`} key={title}><i>{index < compileStep ? "✓" : index + 1}</i><div><strong>{title}</strong><small>{note}</small></div></div>)}</div><div className="compile-progress"><i style={{ width: `${((compileStep + 1) / 3) * 100}%` }} /></div><small className="compile-note">路径运行不依赖现场生成；AI 仅在结果后由你主动调用。</small></div></section>}</><main className="shell demo-run-shell program-shell"><header className="topbar demo-topbar"><Link href="/" className="brand">用<span>一下</span></Link><div className="demo-top-label">{prototype ? "产品体验样例" : "知乎知识"}</div>{prototype ? <div className="source-nav-link">知识正在运行</div> : <Link className="source-nav-link" href={`/knowledge/${run.sourceRef.sourceId}`}>阅读原文 ↗</Link>}</header><div className="container demo-run-container">
<section className="run-console demo-run-hero program-run-hero"><div className="console-copy"><div className="eyebrow console-eyebrow">用一下这篇知识</div><h1 className="run-title">{run.title}</h1><p className="lede">{run.description}</p><div className="official-run-promise"><span><small>带着什么来</small><strong>{experience.bring}</strong></span><i>→</i><span><small>这次会拿走</small><strong>{experience.take}</strong></span></div><p className="source-byline"><span className="source-pulse" />{prototype ? `交互结构样例：${modelName(model)}` : `知识源已连接：${source.title} · ${source.authorName}`}</p>{!prototype && <SourceScopeNotice source={source} />}{prototype && <p className="prototype-run-notice">这是产品交互样例，用于验证状态、路径和结果体验；不代表真实知乎内容。</p>}</div><div className="program-hero-side"><div className="program-run-orb"><div className="demo-hero-mark"><span>RUN</span><strong>{sequence}</strong></div></div><div className="run-usage"><span>▶ {usage.started} 次运行</span><span>✓ {usage.completed} 次完成</span></div></div></section>
    <div className="run-guide program-run-guide"><div><span>这次你会带走</span><strong>{experience.take}</strong></div><div><span>现在要做</span><p>带入你的真实情况，按这篇知识的规则完成判断。</p></div><b>{result ? "结果已生成" : completed > 0 ? `正在运行你的情况 · 第 ${completed + 1} 步` : "从真实情况开始"}</b></div>
